    ...
    class Handler_Incidents
    {
        ...
        public  Operationlogs Add_Incident(Users u, Incidents i)//this transaction requires User_Role_Ahwal Permisson on this AhwalID
        {
            try
            {
                ...
                //by default,we will add new comment for a new incidet,this will help later in the incidents dedicated page, that each Incidents at least has one comment

                _context.SaveChanges();
                //still, the Incidents status will be new, but with this we make sure that each Incidents at least has one comment
